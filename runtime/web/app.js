...
function render() {
  ...
  <aside class="sidebar">
    ...
    <div class="history"><button class="history-item ${report.demo ? "selected" : ""}" data-action="demo"><span class="history-dot"></span><span>AI 音乐视频生成器<small>示例研究 · 合成数据</small></span></button>${history.map(h => `<button class="history-item ${h.id === report.id ? "selected" : ""}" data-report="${esc(h.id)}"><span class="history-dot"></span><span>${esc(h.query)}<small>${date(h.createdAt)}</small></span></button>`).join("")}</div>
